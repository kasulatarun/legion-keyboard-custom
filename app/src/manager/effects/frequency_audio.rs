...
pub fn play(manager: &mut Inner, profile: &Profile, sensitivity: f32) {
    ...
    let device = {
        ...
        devices
            .find(|d| d.name().map(|n| n.contains("Default") || n.contains("Stereo Mix")).unwrap_or(false))
    ...
    while !stop_signals.manager_stop_signal.load(Ordering::SeqCst) {
        ...
        let samples = if let Ok(buf) = audio_buffer.lock() { buf.clone() } else { vec![0.0; FFT_SIZE] };
